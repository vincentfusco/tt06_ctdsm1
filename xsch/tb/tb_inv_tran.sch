...
C {devices/lab_pin.sym} 100 0 2 0 {name=p1 sig_type=analog lab=vout}
C {devices/gnd.sym} -80 120 0 0 {name=l3 lab=GND}
C {devices/lab_pin.sym} -80 0 0 0 {name=p2 sig_type=analog lab=vin}
C {devices/vsource.sym} -300 70 0 0 {name=VDD value=vdda savecurrent=false}
...
.param vdda=1.8
"}
C {devices/code.sym} -210 330 0 0 {name=CONTROL only_toplevel=false value=
"
.control
	set wr_vecnames
	set wr_singlescale
	...
	dowhile run <= mc_runs
		dc Vtest 0 1.8 0.01
		wrdata inverter\{$&run\}.raw v(vout)
		reset
		let run = run + 1
	end
.endc
